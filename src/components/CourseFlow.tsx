...
export function CourseFlow({ courseId, courseName, steps, quiz, onComplete }: CourseFlowProps) {
  ...
  if (done) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-8">
        ...
        <h2 className="text-3xl font-black text-gray-800 mb-3 font-heading">
          You&apos;re ready!
        </h2>
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          ...
          <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full text-lg font-black mt-1 mb-2 text-white font-heading"
            style={{ background: passed ? 'linear-gradient(135deg, #10B981, #059669)' : 'linear-gradient(135deg, #F59E0B, #D97706)' }}>
            ...
            return (
              <div key={i} className={`rounded-2xl p-5 border ${isCorrect ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-bold text-gray-800 text-sm mb-2 font-heading">{q.question}</p>
                    {!isCorrect && (
                      <p className="text-xs text-red-600 mb-1">
                        You answered: <span className="font-semibold">{q.options[userAnswer] ?? 'No answer'}</span>
                      </p>
                    )}
                    <p className={`text-xs font-semibold ${isCorrect ? 'text-emerald-700' : 'text-gray-700'}`}>
                      ✓ {q.options[correct]}
                    </p>
        ...
        {passed ? (
          <button
            ...
            disabled={completing}
            className="w-full py-4 rounded-full text-white font-black text-lg shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all disabled:opacity-70 font-heading bg-brand"
          >
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 py-3 rounded-full border-2 border-pink-300 text-pink-600 font-bold hover:bg-pink-50 transition-colors font-heading"
            >
              Re-read lesson
            </button>
            <button
              onClick={() => { setAnswers({}); setPhase('quiz') }}
              className="flex-1 py-3 rounded-full text-white font-bold shadow-md hover:shadow-lg hover:scale-[1.02] transition-all font-heading bg-brand"
            >
              Try again 💪
            </button>
          </div>
  ...
  if (phase === 'quiz' && quiz) {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-purple-50 border border-purple-200 text-purple-600 text-sm font-semibold mb-4">
            🧠 {courseName} Quiz
          </div>
          <h1 className="text-2xl font-black text-gray-800 mb-2 font-heading">
        ...
        <div className="space-y-6 mb-8">
          {quiz.map((q, qi) => (
            <div key={qi} className="bg-white rounded-2xl p-6 shadow-sm border border-pink-50">
              <p className="font-bold text-gray-800 mb-4 text-sm font-heading">
                <span className="inline-flex items-center justify-center w-6 h-6 rounded-full text-white text-xs font-black mr-2 bg-brand">
                  {qi + 1}
                </span>
                {q.question}
              </p>
        ...
        <button
          ...
          className="w-full py-4 rounded-full text-white font-black text-lg shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all disabled:opacity-40 disabled:scale-100 disabled:cursor-not-allowed font-heading bg-brand"
        >
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      {/* Header */}
      <div className="text-center mb-8">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-pink-50 border border-pink-200 text-pink-600 text-sm font-semibold mb-4">
          📚 {courseName} Course
        </div>
        <h1 className="text-2xl font-black text-gray-800 mb-2 font-heading">
          Before you dive in...
      ...
      <div className="bg-white rounded-3xl shadow-lg border border-pink-50 p-8 mb-6">
        ...
        <h2 className="text-2xl font-black text-gray-800 text-center mb-4 font-heading">
          {step.title}
        </h2>
      ...
      <div className="flex items-center justify-between gap-4">
        <button
          ...
          className="flex items-center gap-2 px-5 py-3 rounded-full text-gray-500 font-semibold border border-gray-200 hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition-all font-heading"
        >
          <ChevronLeft size={18} />
          Back
        </button>
